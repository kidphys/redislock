...
public class JedisLock implements ITransactionLock {
	...
	public JedisLock(TimeoutTracker timeoutTracker, String key, String identity, JedisPool pool){
	...
	private void validateTimeUnit(TimeUnit unit) {
		if (unit != TimeUnit.MILLISECONDS && unit != TimeUnit.SECONDS) {
			throw new IllegalArgumentException("Only accept MILLISECONDS or SECONDS");
		}
	}
	...
	public Boolean tryLock() {
		...
		try {
			...
			String result = jedis.set(key, tempLastHoldValue, "NX", "PX", expireTime);
			if ("OK".equals(result)) {
